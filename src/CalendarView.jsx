...
export default function CalendarView({ themeToggle, timerIsland, syncTrigger }) {
  ...
  // 🔥 EXPLICIT SYNC TRIGGERS ADDED HERE 🔥
  useEffect(() => { localStorage.setItem('tracker-chapters', JSON.stringify(chapters)); if(syncTrigger) syncTrigger(); }, [chapters, syncTrigger]);
  useEffect(() => { localStorage.setItem('tracker-events', JSON.stringify(events)); if(syncTrigger) syncTrigger(); }, [events, syncTrigger]);
  useEffect(() => { localStorage.setItem('tracker-mocks', JSON.stringify(mocks)); if(syncTrigger) syncTrigger(); }, [mocks, syncTrigger]);
  ...
  const handleSaveTask = () => {
    ...
    const newEvent = { id: editingId || String(Date.now()), title: taskName, start: `${selectedDate}T${startTime}:00`, end: `${selectedDate}T${endTime}:00`, colorKey: taskColorKey, subject, linkedChapterTitle, allDay: false, done: existingTask ?
    existingTask.extendedProps?.done || existingTask.done : false };
  ...
  const openMockModal = (existingMock = null) => {
    ...
    if (existingMock) { setMockEditId(existingMock.id);
      setMockForm({ ...existingMock, selectedChapters: existingMock.selectedChapters || [] }); } 
    else { setMockEditId(null);
      setMockForm({ name: '', date: formatLocalYMD(new Date()), start: '09:00', end: '12:00', type: 'JEE Mains', selectedChapters: [], isCompleted: false, score: null });
    }
  ...
  const saveMockTest = () => {
    if(!mockForm.name || !mockForm.date || !mockForm.start || !mockForm.end || !mockForm.type) {
      alert("⚠️ CAUTION: Please fill all mandatory fields (Name, Date, Type, Start Time, and End Time) before saving.");
      return;
    }
  ...
  const deleteTask = (id) => { setEvents(events.filter(e => e.id !== id));
  setDailyModal(prev => ({ ...prev, tasks: prev.tasks.filter(t => t.id !== id) })); };
  ...
  const toggleChapterDone = (id) => {
    setChapters(chapters.map(c => {
      if (c.id === id) {
        ...
        setFullSyllabus(updatedFullSyllabus); 
        localStorage.setItem('tracker-syllabus', JSON.stringify(updatedFullSyllabus));
        // 🔥 Trigger sync directly here for syllabus change
  ...
  const renderEventContent = (eventInfo) => {
    ...
    eventInfo.event.start.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', hour12: true}) : '';
    const e = eventInfo.event.end ?
    eventInfo.event.end.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', hour12: true}) : '';
    return (
    ...
    return (
      <div className={`flex justify-between items-start w-full h-full p-1 cursor-pointer group transition-all duration-300 ${isPast ? 'bg-slate-50/10 dark:bg-slate-800/20 opacity-50 hover:opacity-100' : ''}`} onClick={() => triggerDailyModal(dateStr)}>
          ...
          {dayMocks.length > 0 && currentView === 'dayGridMonth' && (
             <div className="flex items-center gap-1 z-20">
              ...
              <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-80 ${allCompleted ? 'bg-green-400' : 'bg-red-400'}`}></span>
                ...
                {dayMocks.length > 1 && <span className={`text-[10px] font-black leading-none ${allCompleted ? 
                'text-green-500' : 'text-red-500'}`}>+{dayMocks.length - 1}</span>}
             </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-1 pt-0.5 pr-0.5">
          ...
          openAddModal(dateStr); }} className="relative z-50 opacity-0 group-hover:opacity-100 transition-opacity p-1 bg-white/40 border border-white/20 dark:border-white/10 dark:bg-white/10 rounded-full text-slate-600 dark:text-slate-300 shadow-sm backdrop-blur-md">
            <Plus size={14} className="pointer-events-none" />
          </button>
          <div className={`flex items-center justify-center w-7 h-7 rounded-full transition-colors ${isToday ?
          'bg-blue-600 text-white shadow-md z-10' : 'text-slate-700 dark:text-slate-200 group-hover:bg-slate-200/50 dark:group-hover:bg-slate-700/50 z-10'}`}>
  ...
  const getMiniCalendarGrid = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) { currentRow.push(new Date(year, month, i)); if (currentRow.length === 7) { grid.push(currentRow);
    currentRow = []; } }
       ...
       {openDropdown === typeKey && (
          <div className="absolute top-full left-0 mt-1 w-full bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border border-blue-200 dark:border-blue-800 rounded-lg shadow-2xl z-[300] max-h-48 overflow-y-auto hide-scrollbar">
             {options.map(opt => (
                ...
                setOpenDropdown(null); }} className="px-4 py-3 text-sm font-bold text-slate-700 dark:text-slate-300 hover:bg-blue-50 dark:hover:bg-blue-900/30 cursor-pointer border-b border-slate-100 dark:border-slate-800 last:border-0">{typeKey === 'date' ?
                getDateLabel(opt) : opt}</div>
             ))}
          </div>
       )}
    </div>
    ...
    <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl mini-h-screen w-full flex flex-col transition-colors duration-300 relative rounded-[32px] shadow-2xl border border-white/20 mb-2 mr-2">
      ...
      <div className="flex justify-between items-center px-6 py-4 border-b border-slate-300/40 dark:border-slate-700/50 shrink-0">
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            bg-white/40 dark:bg-white/10 backdrop-blur-md py-1.5 px-3 rounded-full border border-white/20 dark:border-white/5 shadow-sm" onClick={() => setIsDateDropdownOpen(!isDateDropdownOpen)}>
            ...
            {isDateDropdownOpen && (
              <div className="absolute top-full left-0 mt-2 w-72 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-[24px] 
              shadow-2xl border border-white/20 dark:border-white/10 z-[200] p-2 max-h-[350px] overflow-y-auto hide-scrollbar">
        ...
        <div className="flex items-center gap-3 transition-all duration-500">
          <div className="relative flex w-[280px] bg-slate-300/30 dark:bg-black/30 backdrop-blur-xl p-1.5 rounded-full shadow-[inset_0_2px_8px_rgba(0,0,0,0.1)] border border-white/40 dark:border-white/5 z-0">
            <div className="absolute top-1.5 bottom-1.5 
            rounded-full bg-white/90 dark:bg-white/20 shadow-md border border-white/60 dark:border-white/30 transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] z-0"
      ...
      <div className="flex-1 flex flex-col md:flex-row bg-transparent relative w-full">
        ...
        <div className="w-full md:w-[300px] border-b md:border-b-0 md:border-r border-slate-300/40 dark:border-slate-700/50 p-5 flex flex-col hidden md:flex shrink-0">
                ...
                return (
                  <div key={task.id} onClick={() => toggleTaskDone(task.id)} className="flex items-start gap-3 cursor-pointer group p-3 bg-white/30 dark:bg-black/20 border border-white/30 dark:border-white/5 rounded-2xl shadow-sm hover:bg-white/40 dark:hover:bg-black/40 transition-colors">
                    ...
                    <div className="flex flex-col">
                       ...
                       <div className={`text-[10px] font-semibold mt-1 flex items-center gap-1.5 ${isDone ?
                        'text-slate-400/50' : 'text-slate-500 dark:text-slate-400'}`}>
                          <span>{extractTime(task.start, '00:00')} - {extractTime(task.end, '00:00')}</span>
                          <span className="text-[8px]">•</span>
                          <span>{task.extendedProps?.subject ||
                          task.subject || 'Subject'}</span>
                       </div>
                    </div>
                  </div>
                )
          ...
          <div className="mb-6 shrink-0 relative z-20">
            ...
            <div className="flex flex-col gap-3">
              ...
              <span className="text-xs font-medium text-slate-500">No chapters mapped for this month.</span> : currentMonthChapters.map(chap => (
                <div key={chap.id} onClick={() => toggleChapterDone(chap.id)} className={`flex items-start gap-3 cursor-pointer group p-3 border rounded-2xl shadow-sm transition-colors ${chap.done ? 'bg-slate-200/50 dark:bg-slate-800/50 border-transparent opacity-60 grayscale' : 'bg-white/30 dark:bg-black/20 border-white/30 dark:border-white/5 hover:bg-white/40 dark:hover:bg-black/40'}`}>
                  <div className="pt-0.5">
                     {chap.done ? <CheckSquare size={16} 
                     className="text-blue-500 flex-shrink-0" /> : <Square size={16} className="text-slate-400 dark:text-slate-500 group-hover:text-blue-400 transition-colors flex-shrink-0" />}
                  </div>
                  <div className="flex flex-col">
          ...
          <div className="shrink-0 pb-10 relative z-20">
             <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex items-center gap-2">
                 ...
                 setIsMockHistoryOpen(true)} className="relative z-50 p-1.5 rounded-full bg-white/40 dark:bg-white/10 text-slate-500 dark:text-slate-400 hover:text-red-500 dark:hover:text-red-400 border border-white/20 dark:border-white/5 transition-all hover:scale-110 shadow-sm cursor-pointer" title="View Full History">
                    <History size={14} className="pointer-events-none" />
                 </button>
                 <button type="button" onClick={() => openMockModal()} className="relative z-50 p-1.5 rounded-full bg-white/40 dark:bg-white/10 text-slate-500 dark:text-slate-400 hover:text-red-500 dark:hover:text-red-400 border border-white/20 dark:border-white/5 transition-all hover:scale-110 shadow-sm cursor-pointer" title="Add 
                 Mock">
                    <Plus size={14} className="pointer-events-none" />
                 </button>
              </div>
                ...
                mocks.filter(m => m.date?.startsWith(currentMonthKey)).sort((a,b) => (a.date||'').localeCompare(b.date||'')).map(m => (
                  <div key={m.id} className={`p-4 rounded-2xl border shadow-sm relative overflow-hidden backdrop-blur-md transition-colors ${getMockBoxBgClass(m)}`}>
                    ...
                    <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2">
                            {m.isCompleted ? (
                                ...
                                openScoreModal(m); }} className="group flex items-center justify-center w-5 h-5 flex-shrink-0 cursor-pointer relative z-50" title="Mark as Done">
                                   ...
                                   <div className="absolute top-[-2px] right-[-2px] w-2.5 h-2.5 bg-red-500 
                                   rounded-full animate-ping opacity-80"></div>
                        ...
                        {m.isCompleted && (
                            <div className={`text-[10px] font-black px-2 py-0.5 rounded-lg border shadow-sm ${getScorePillClass(m)}`}>
                                {m.score}/{m.type === 'JEE Mains' ?
                                300 : 360}
                            </div>
                        )}
                    </div>
                    <div className="text-[10px] font-bold text-white/80 mt-1 pl-7">{m.date} 
                    | {m.type}</div>
                    <div className="flex flex-wrap gap-1.5 mt-3 pl-7">
                        ...
                        {m.selectedChapters && m.selectedChapters.length > 3 && <span className="text-[9px] font-black text-white/80 
                        mt-1">+{m.selectedChapters.length - 3}</span>}
                    </div>
                  </div>
                ))
              )}
        ...
        <div className="custom-calendar relative p-4 h-full flex-1 flex flex-col z-0 overflow-hidden w-full">
          <style>{`
            .custom-calendar .fc { --fc-border-color: rgba(148, 163, 184, 0.2);
            --fc-button-bg-color: rgba(59, 130, 246, 0.1); --fc-button-border-color: transparent; --fc-button-hover-bg-color: rgba(59, 130, 246, 0.2); --fc-button-active-bg-color: rgba(59, 130, 246, 0.3);
            --fc-today-bg-color: rgba(59, 130, 246, 0.05); height: 100%; width: 100%;}
            .custom-calendar .fc-toolbar-title { font-size: 1.25rem;
            font-weight: 800; color: inherit; }
            .custom-calendar .fc-col-header-cell { padding: 12px 0;
            font-size: 0.8rem; text-transform: uppercase; font-weight: 800; opacity: 0.7; }
            .custom-calendar .fc-daygrid-day-number { width: 100%;
            padding: 0; }
            
            .custom-calendar .fc-daygrid-day-top { display: flex !important;
            flex-direction: row; justify-content: space-between; width: 100%; margin-bottom: 6px; }
            
            .custom-calendar .fc-daygrid-event-harness { margin-top: 4px;
            }
            .custom-calendar .fc-event { background: transparent; border: none;
            padding: 0; margin-bottom: 4px !important; }
          ...
          <div className="w-full h-full">
            <FullCalendar
              ...
              setCurrentRenderDate(arg.view.currentStart); }} 
              dateClick={handleDateClick}
              eventClick={(arg) => openAddModal(arg.event.startStr, events.find(e => e.id === arg.event.id))}
              dayCellContent={currentView === 'dayGridMonth' ?
              renderMonthCell : undefined}
              dayHeaderContent={renderHeaderContent}
              moreLinkClick={(arg) => { triggerDailyModal(formatLocalYMD(arg.date));
              return 'prevent'; }}
                      ...
                      <div key={m.id} className={`p-5 rounded-2xl border shadow-sm flex flex-col md:flex-row gap-4 md:items-center justify-between ${getMockBoxBgClass(m)}`}>
                         <div className="flex-1">
                            ...
                            <div className="mt-3 flex flex-wrap gap-1.5">
                               ...
                               {m.selectedChapters?.map(id => <span key={id} className="bg-black/10 dark:bg-white/10 text-[10px] font-bold px-2 py-1 rounded-md border border-black/5 dark:border-white/5">{fullSyllabus.find(s=>s.id===id)?.title ||
                               'Ch'}</span>)}
                            </div>
                         </div>
                         <div className="flex gap-2">
                    
                           <button onClick={() => { setIsMockHistoryOpen(false); openMockModal(m);
                           }} className="p-2 hover:bg-black/10 dark:hover:bg-white/10 rounded-xl transition-colors"><Pencil size={16} className="text-slate-500" /></button>
      ...
      {isScoreModalOpen && activeMockForScore && (
         ...
         bg-slate-900/60 backdrop-blur-md z-[99999] flex justify-center items-center p-4">
            <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-3xl w-full max-w-sm rounded-[32px] shadow-2xl p-8 border border-white/20 relative animate-in zoom-in-95">
               ...
               <p className="text-sm 
               font-bold text-slate-500 mb-8">{activeMockForScore.name} ({activeMockForScore.type})</p>
               ...
               <div className="mb-10">
                  ...
                  <div className="flex items-center justify-center gap-4 bg-slate-100/80 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 w-full overflow-hidden shadow-inner">
                     ...
                     <span className="text-2xl font-black text-slate-400 whitespace-nowrap">/ {activeMockForScore.type === 'JEE Mains' ?
                     300 : 360}</span>
                  </div>
               </div>
      ...
      {isMockModalOpen && (
        <div className="fixed inset-0 z-[99999] flex justify-center items-center p-4">
          <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-3xl w-full max-w-xl rounded-[32px] shadow-2xl p-8 border border-white/20 max-h-[90vh] overflow-y-auto hide-scrollbar relative">
             ...
             <h3 className="text-2xl font-black mb-6 flex items-center gap-3 text-slate-800 dark:text-white"><Target className="text-blue-500"/> {mockEditId ?
             'Edit Mock Test' : 'Schedule Mock Test'}</h3>
             ...
             <div className="space-y-5 relative">
                ...
                <div className="flex gap-4">
                  <CustomDropdown label="Date" 
                  value={mockForm.date} options={NEXT_14_DAYS} typeKey="date" onChange={(v) => setMockForm({...mockForm, date: v})} />
                ...
                <div className="flex gap-4">
                  <CustomDropdown label="Start Time" value={mockForm.start} options={TIME_OPTIONS} typeKey="start" onChange={(v) => setMockForm({...mockForm, 
                  start: v})} />
                ...
                <div>
                   ...
                   <div className="flex gap-2 mb-3">
                     {['Physics', 'Chemistry', 'Mathematics'].map(subj => (
                        <button type="button" key={subj} onClick={() => setMockSubjectTab(mockSubjectTab === subj ?
                        null : subj)} className={`flex-1 py-2 rounded-xl text-xs font-bold transition-colors shadow-sm ${mockSubjectTab === subj ?
                        'bg-blue-500 text-white shadow-md' : 'bg-slate-200/50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 hover:bg-slate-300/50 dark:hover:bg-slate-700/50'}`}>{subj}</button>
                     ))}
                   </div>

                   {mockSubjectTab && (
                      <div className="bg-slate-100/80 dark:bg-slate-800/80 p-4 rounded-xl border border-slate-200 dark:border-slate-700 
                      max-h-[200px] overflow-y-auto mb-3 hide-scrollbar shadow-inner">
                   ...
                   flex-wrap gap-2 mt-2">
                     ...
                     {(!mockForm.selectedChapters ||
                     mockForm.selectedChapters.length === 0) && <span className="text-xs text-slate-400 font-medium italic">No chapters selected.</span>}
                   </div>
                </div>
      ...
      {isModalOpen && (
        <div className="absolute inset-0 z-50 flex justify-center items-center p-4">
          <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-3xl w-full max-w-md rounded-[32px] p-8 shadow-2xl border border-white/20 dark:border-white/10 text-slate-800 dark:text-white relative max-h-[90vh] overflow-y-auto hide-scrollbar">
            ...
            <h3 className="text-2xl font-black mb-8 tracking-tight flex items-center gap-3"><CalIcon className="text-blue-500"/> {editingId ?
            'Edit Task' : 'New Task'}</h3>
            ...
            <div className="flex bg-slate-50/50 dark:bg-slate-800/30 rounded-3xl 
            p-4 mb-6 border border-slate-200 dark:border-slate-700/50 h-[150px]">
              <div className="flex-1 flex flex-col items-center">
                ...
                <div className="w-full overflow-y-auto snap-y snap-mandatory hide-scrollbar flex-1 relative">
                  ...
                  {TIME_OPTIONS.map(time => (<div key={`start-${time}`} onClick={() => setStartTime(time)} className={`h-[40px] flex items-center justify-center snap-center cursor-pointer transition-all ${startTime === time ?
                  'text-blue-600 dark:text-white text-xl font-black bg-white dark:bg-white/10 rounded-xl shadow-sm border border-slate-200 dark:border-transparent' : 'text-slate-400 text-sm font-bold hover:text-slate-600 dark:hover:text-slate-300'}`}>{time}</div>))}
                  <div className="h-[40px]"></div>
                </div>
              </div>
              ...
              flex flex-col items-center">
                ...
                <div className="w-full overflow-y-auto snap-y snap-mandatory hide-scrollbar flex-1 relative">
                  <div className="h-[40px]"></div>
                  {TIME_OPTIONS.map(time => (<div key={`end-${time}`} onClick={() => setEndTime(time)} className={`h-[40px] flex items-center justify-center snap-center cursor-pointer transition-all ${endTime === time 
                  ? 'text-blue-600 dark:text-white text-xl font-black bg-white dark:bg-white/10 rounded-xl shadow-sm border border-slate-200 dark:border-transparent' : 'text-slate-400 text-sm font-bold hover:text-slate-600 dark:hover:text-slate-300'}`}>{time}</div>))}
                  <div className="h-[40px]"></div>
                </div>
              </div>
             ...
             <label className="text-[11px] font-black text-slate-400 tracking-widest uppercase mb-3 block">Subject</label>
              <div className="flex gap-2">
                {['Physics', 'Chemistry', 'Mathematics'].map(sub => (
                  <button type="button" key={sub} onClick={() => { setSubject(sub);
                  setLinkedChapterTitle(''); }} className={`flex-1 py-3 rounded-2xl text-xs font-extrabold border transition-colors ${subject === sub ?
                  'bg-blue-500/20 border-blue-500 text-blue-600 dark:text-blue-400 shadow-sm' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
                    {sub === 'Mathematics' ?
                    'Maths' : sub}
                  </button>
                ))}
              ...
              <div className="flex flex-wrap gap-3">
                {Object.entries(TASK_COLORS).map(([key, vals]) => (
                  <button 
                    ...
                    className={`w-8 h-8 rounded-full shadow-sm transition-all border-2 ${taskColorKey === key ?
                    'ring-4 ring-offset-2 dark:ring-offset-[#0f172a] scale-110' : 'hover:scale-110 opacity-70 hover:opacity-100'}`} 
         ...
         <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-3xl w-full max-w-lg rounded-[32px] p-8 shadow-2xl border border-white/20 dark:border-white/10" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-8">
              ...
              <div className="flex gap-3">
                <button type="button" 
                onClick={() => openAddModal(dailyModal.dateStr)} className="p-2.5 bg-blue-500/10 text-blue-600 dark:text-blue-400 hover:bg-blue-500 hover:text-white rounded-full transition-colors"><Plus size={20} /></button>
            ...
            pr-2 hide-scrollbar">
              ...
              <div>
                 ...
                 <div className="space-y-3">
                   {dailyModal.tasks.length === 0 ?
                   <p className="text-slate-500 text-center py-4 font-bold text-sm bg-slate-50/50 dark:bg-slate-800/30 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">No tasks scheduled.</p> : (
                     dailyModal.tasks.sort((a,b) => (a.start||'').localeCompare(b.start||'')).map(task => {
                       const k = task.colorKey || 'blue';
                       const isDone = task.extendedProps?.done !== undefined ? task.extendedProps.done 
                       : task.done;
                       const c = TASK_COLORS[k] || TASK_COLORS.blue;
                       return (
                         ...
                         ''}`}>
                           ...
                           <div className="flex items-center gap-3">
                             ...
                             px-2 py-1 rounded-lg opacity-80 bg-black/10" style={{ color: c.text }}>{extractTime(task.start, '00:00')} - {extractTime(task.end, '00:00')}</span>
                ...
                <div>
                   ...
                   <div className="space-y-3">
                      {dailyModal.dayMocks.sort((a,b) => (a.start||'').localeCompare(b.start||'')).map(m => (
                          <div key={m.id} className="flex justify-between items-center p-4 rounded-2xl shadow-sm transition-transform hover:scale-[1.02] bg-white/30 dark:bg-black/20 
                          border border-white/30 dark:border-white/5">
                             ...
                             <div className="flex items-center gap-3">
                               ...
                               font-black px-2 py-1 rounded-lg bg-slate-200/50 dark:bg-slate-700/50 text-slate-600 dark:text-slate-300">{m.start} - {m.end}</span>
                               <div className="flex gap-1 ml-2">
        ...
        <div className="fixed inset-0 z-[99999] flex justify-center items-center p-4">
          <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-3xl w-full max-w-5xl rounded-[32px] p-8 shadow-2xl border border-white/20 dark:border-white/10 text-slate-800 dark:text-white relative flex flex-col max-h-[85vh]">
            ...
            pr-2 hide-scrollbar">
                      ...
                      <div className="grid gap-2 overflow-y-auto max-h-[50vh] hide-scrollbar pr-2">
                        ...
                        {subjChapters.map(ch => {
                          ...
                          <div key={ch.id} onClick={() => toggleChapterSelection(ch.id)} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-colors border ${isSelected ? 'bg-blue-500/10 border-blue-500/30' : 'bg-slate-100/50 dark:bg-slate-800/30 border-transparent hover:bg-slate-200/50 dark:hover:bg-slate-800/60'}`}>
                              ...
                              <span className={`text-sm font-bold leading-tight ${isSelected ?
                              'text-blue-600 dark:text-blue-400' : 'text-slate-700 dark:text-slate-300'}`}>{ch.title}</span>
                            </div>
                          )
                        })}
              ...
              {fullSyllabus.length === 0 && <div className="text-center text-slate-500 font-bold py-8 mt-4 bg-slate-100/50 dark:bg-slate-800/30 rounded-2xl">Your syllabus is empty.
              Add chapters in the Syllabus tab first.</div>}
            </div>
